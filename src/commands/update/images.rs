...
const SITE_LINK: &str = "https://www.dustloop.com/wiki/api.php?action=cargoquery&format=json&limit=100&tables=MoveData_GGST&fields=MoveData_GGST.input%2C%20MoveData_GGST.name%2C%20MoveData_GGST.images%2C%20MoveData_GGST.hitboxes%2C%20MoveData_GGST.hitboxCaption&where=chara%3D%22";
...
pub async fn get_char_images(chars_ids: [&str; CHARS.len()], specific_char: &str, agent: &ureq::Agent) {
    ...
    if specific_char == "all" {
        ...
        for (x, char_id) in chars_ids.iter().enumerate() {
            ...
            let char_images_response_json = char_images_response_json.into_body().read_to_string().unwrap();
