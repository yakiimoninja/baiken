...
pub async fn info_to_json(mut char_info_response_json: String, mut file: &File, char_count: usize){
    ...
    let processed_char_info = serde_json::to_string_pretty(&CharInfo {
        defense: char_info.defense.as_ref().unwrap_or_else(|| &empty).to_string(),
        guts: char_info.guts.as_ref().unwrap_or_else(|| &empty).to_string(),
        guard_balance: char_info.guard_balance.as_ref().unwrap_or_else(|| &empty).to_string(),
        prejump: char_info.prejump.as_ref().unwrap_or_else(|| &empty).to_string(),
        umo: _umo_processed_string,
        forward_dash: char_info.forward_dash.as_ref().unwrap_or_else(|| &empty).to_string(),
        backdash: char_info.backdash.as_ref().unwrap_or_else(|| &empty).to_string(),
        backdash_duration: char_info.backdash_duration.as_ref().unwrap_or_else(|| &empty).to_string(),
        backdash_invincibility: char_info.backdash_invincibility.as_ref().unwrap_or_else(|| &empty).to_string(),
        backdash_airborne: char_info.backdash_airborne.as_ref().unwrap_or_else(|| &empty).to_string(),
        backdash_distance: char_info.backdash_distance.as_ref().unwrap_or_else(|| &empty).to_string(),
        jump_duration: char_info.jump_duration.as_ref().unwrap_or_else(|| &empty).to_string(),
        jump_height: char_info.jump_height.as_ref().unwrap_or_else(|| &empty).to_string(),
        high_jump_duration: char_info.high_jump_duration.as_ref().unwrap_or_else(|| &empty).to_string(),
        high_jump_height: char_info.high_jump_height.as_ref().unwrap_or_else(|| &empty).to_string(),
        earliest_iad: char_info.earliest_iad.as_ref().unwrap_or_else(|| &empty).to_string(),
        ad_duration: char_info.ad_duration.as_ref().unwrap_or_else(|| &empty).to_string(),
        ad_distance: char_info.ad_distance.as_ref().unwrap_or_else(|| &empty).to_string(),
        abd_duration: char_info.abd_duration.as_ref().unwrap_or_else(|| &empty).to_string(),
        abd_distance: char_info.abd_distance.as_ref().unwrap_or_else(|| &empty).to_string(),
        movement_tension: char_info.movement_tension.as_ref().unwrap_or(&empty).to_string(),
        jump_tension: char_info.jump_tension.as_ref().unwrap_or_else(|| &empty).to_string(),
        airdash_tension: char_info.airdash_tension.as_ref().unwrap_or_else(|| &empty).to_string(),
        walk_speed: char_info.walk_speed.as_ref().unwrap_or_else(|| &empty).to_string(),
        back_walk_speed: char_info.back_walk_speed.as_ref().unwrap_or_else(|| &empty).to_string(),
        dash_initial_speed: char_info.dash_initial_speed.as_ref().unwrap_or_else(|| &empty).to_string(),
        dash_acceleration: char_info.dash_acceleration.as_ref().unwrap_or_else(|| &empty).to_string(),
        dash_friction: char_info.dash_friction.as_ref().unwrap_or_else(|| &empty).to_string(),
        jump_gravity: char_info.jump_gravity.as_ref().unwrap_or_else(|| &empty).to_string(),
        high_jump_gravity: char_info.high_jump_gravity.as_ref().unwrap_or_else(|| &empty).to_string(),
    }).unwrap();
